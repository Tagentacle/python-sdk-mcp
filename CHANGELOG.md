...
### Changed
- **CI**: Migrated from pip to uv. Removed test job (Layer 2, needs daemon). GHA now lint + build only (Layer 1). Fixed sdk-core checkout path for uv.sources.
- **`.gitignore`**: Added `.ruff_cache/`, `.pytest_cache/`.

## [0.4.0] - 2026-03-03

### Added
- **TACL `space` claim** — JWT payload now supports an optional `space` field identifying the agent's isolated execution environment (e.g., Docker container name).
  - `CallerIdentity` gains `space: Optional[str]` attribute.
  - `sign_credential()` accepts optional `space` parameter; included in JWT if provided.
  - `TACLAuthMiddleware` extracts `space` from verified payload and sets it on `CallerIdentity`.
  - `PermissionMCPServerNode.register_agent` accepts optional `space` parameter; stored in agents DB.
  - `PermissionMCPServerNode.authenticate` includes `space` in signed JWT.
  - `agents` table schema gains `space TEXT` column.
  - Bus service `/tagentacle/permission/register_agent` forwards `space`.
